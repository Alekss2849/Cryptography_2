...
//Here we make xor for some lines to remove key from it
//Then we make xor with some words to delete them from xor(addition)
//For example: if we have "We go to the street", "Here is the comment" and they're ciphered
//First: add their ciphers with xor. Then try some words in different positions. Here if we apply "            comment" to xor we add "...(some trash)... street".
...
getBytesFromCipher = hexString => hexString.match(/.{1,2}/g).map(n => parseInt(n, 16));
...
const bytesXor = (arr1, arr2) => {
    let [longer, shorter] = arr1.length > arr2.length ? [arr1, arr2] : [arr2, arr1];
    return longer.map((a, b) => shorter[b] === undefined ? a : a ^ shorter[b]);
}
...
consoleGuessing = () =>{
    ...
    console.log('\n\n');
    while(true){
        const index1 = prompt('Index of first text(0-18): ');
        const index2 = prompt('Index of second text(0-18): ');
        const xorWords = prompt('Word to xor with: ');
        if(!index1 || !index2 || !xorWords){
            break;
        }

        if(index1 < 0 || index1 > startText.length - 1 ||
           index2 < 0 || index2 > startText.length - 1){
            console.log('Wrong input\n');
            continue;
        }

        let xored = bytesXor(TEXT_BYTES[index1], TEXT_BYTES[index2]);
        //Make array of keys in different positions
        const xorWordsPositions = [...Array(xored.length).keys()].map(n => ' '.repeat(n) + xorWords);
        //Map words in different positions with previously xored ciphers
        xored = xorWordsPositions.map(words => bytesXor(xored, getBytesFromText(words)));
        console.log(xored.map(getTextFromBytes).join('\n'));
    }
}
